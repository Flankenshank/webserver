...
export async function chirpDeleteHandler(req: express.Request, res: express.Response) {
    ...
    if (req.params.chirpId) {
        ...
        await db.delete(chirps).where(eq(chirps.id, req.params.chirpId as string));
        res.status(204).send();
        return;
    }
